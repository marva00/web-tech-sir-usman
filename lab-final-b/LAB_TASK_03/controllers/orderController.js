...
exports.orderPreview = (req, res) => {
    ...
    const discount = req.discount || {
        ...
        discountedTotal: cart.reduce((sum, item) => sum + (item.price * item.quantity), 0),
        discountAmount: 0
    };
...
exports.getMyOrders = async (req, res) => {
    try {
        ...
        if (email) {
            orders = await Order.find({ customerEmail: email }).sort({ createdAt: -1 });
        }
        ...
        res.redirect('/?error=Failed to fetch orders');
    }
};
